...
class SaliencyGuidedRLMix(VanillaMixupPatchDiscrete):
    def __init__(self, 
        ...
        self.action_space_mapper = np.linspace(0, 1, args.num_action)
    ...
    def step(self, action: np.ndarray):
        ...
        actions = torch.as_tensor(actions, dtype=torch.float32, device="cuda").view(1, self.args.num_patches, self.args.num_patches)
        lam = F.interpolate(actions.unsqueeze(0), scale_factor=self.patch_size).squeeze(0)
        ...
        # if reaches the end
            ...
            self.current_perm = self.compute_saliency(img[self.index_perm], targets[self.index_perm], self.model, self.patch_size)
